...
fn b_spline_basis(i: usize, k: usize, t: f32, knots: &[f32]) -> f32 {
    if k == 0 {
        if t >= knots[i] && t < knots[i + 1] { 1.0 } else { 0.0 }
    } else {
...
impl AsMesh for NurbsSurface {
    fn as_mesh(&self) -> Mesh {
        ...
        let gen_knots = |n: usize, k: usize| -> Vec<f32> {
            ...
            for i in 0..m {
                if i < k + 1 { knots[i] = 0.0; }
                else if i >= m - (k + 1) { knots[i] = 1.0; }
                else { knots[i] = (i - k) as f32 / (n - k) as f32; }
            }
            knots
        };
        ...
        for i in 0..=self.splits {
            ...
            for j in 0..=self.splits {
                ...
                for row in 0..n_v {
                    for col in 0..n_u {
                        let idx = row * n_u + col;
                        let nip = b_spline_basis(col, k, u, &u_knots) * b_spline_basis(row, k, v, &v_knots);
