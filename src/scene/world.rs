...
impl RigidBody for CameraObject {
    ...
    fn bounding_volume(&self) -> BoundingVolume {
        ...
        BoundingVolume::AABB(local_aabb.get_global_aabb(self.camera.transform.get_matrix()))
    }
}
...
impl RigidBody for GameObject {
    ...
    fn bounding_volume(&self) -> BoundingVolume {
        match self.mesh.bounding_volume {
            BoundingVolume::AABB(ref aabb) => {
                BoundingVolume::AABB(aabb.get_global_aabb(self.transform.get_matrix()))
            }
            BoundingVolume::Sphere(ref sphere) => BoundingVolume::Sphere(
                sphere.get_global_sphere(self.transform.scale, self.transform.position),
            ),
        }
    }
}
...
impl World {
    ...
    pub fn step(&mut self, dt: f32) {
        ...
        if let Some(idx) = self.get_selected_camera() {
            ...
            camera.current_half_height = lerp(current_height, target, dt * speed);
        }

        for i in 0..bodies.len() {
            if !is_dynamic(bodies[i], self) {
                continue;
            }
    ...
    pub fn init_house_scene(&mut self, _display: &glium::Display<glutin::surface::WindowSurface>) {
        ...
        // 后墙 (实心)
        let mut wall_back = GameObject::new(
            "Wall_Back",
            Box::new(Cube {
                width: room_w + wall_thick * 2.0,
                height: room_h,
                depth: wall_thick,
            }),
            wall_mat,
        ...
        // 右墙 (实心)
        let mut wall_right = GameObject::new(
            "Wall_Right",
            Box::new(Cube {
                width: wall_thick,
                height: room_h,
        ...
        let mut wall_front_header = GameObject::new(
            "Wall_Front_Header",
            Box::new(Cube {
                width: door_total_w,
                height: header_h,
        ...
        wall_front_header.transform.position = [
            0.0,
            door_total_h + header_h / 2.0,
            -room_d / 2.0 - wall_thick / 2.0,
        ]
        .into();
        ...
        let mut wall_front_l = GameObject::new(
            "Wall_Front_L",
            Box::new(Cube {
                width: front_side_w,
                height: room_h,
        ...
        wall_front_l.transform.position = [
            -front_pos_x - 0.2,
            room_h / 2.0,
            -room_d / 2.0 - wall_thick / 2.0,
        ]
        ...
        let mut wall_front_r = GameObject::new(
            "Wall_Front_R",
            Box::new(Cube {
                width: front_side_w,
                height: room_h,
        ...
        wall_front_r.transform.position = [
            front_pos_x + 0.2,
            room_h / 2.0,
            -room_d / 2.0 - wall_thick / 2.0,
        ]
        ...
        let mut wall_left_bot = GameObject::new(
            "Wall_Left_Bot",
            Box::new(Cube {
                width: wall_thick,
                height: win_bottom_h,
        ...
        wall_left_bot.transform.position =
            [-room_w / 2.0 - wall_thick / 2.0, win_bottom_h / 2.0, 0.0].into();
        ...
        let mut wall_left_top = GameObject::new(
            "Wall_Left_Top",
            Box::new(Cube {
                width: wall_thick,
                height: win_top_h,
        ...
        wall_left_top.transform.position = [
            -room_w / 2.0 - wall_thick / 2.0,
            win_top_start + win_top_h / 2.0,
            0.0,
        ]
        .into();
        ...
        // 左墙-前段
        let mut wall_left_front = GameObject::new(
            "Wall_Left_Front",
            Box::new(Cube {
                width: wall_thick,
                height: room_h,
        ...
        wall_left_front.transform.position =
            [-room_w / 2.0 - wall_thick / 2.0, room_h / 2.0, -side_pos_z].into();
        wall_left_front.set_body_type(BodyType::Static);
        self.add_object(wall_left_front);

        // 左墙-后段
        let mut wall_left_back = GameObject::new(
            "Wall_Left_Back",
            Box::new(Cube {
                width: wall_thick,
                height: room_h,
        ...
        wall_left_back.transform.position =
            [-room_w / 2.0 - wall_thick / 2.0, room_h / 2.0, side_pos_z].into();
        ...
        // 门楣框
        let mut d_frame_top = GameObject::new(
            "DoorFrame_Top",
            Box::new(Cube {
                width: door_total_w,
                height: door_frame_thick,
        ...
        // 门左框
        let mut d_frame_l = GameObject::new(
            "DoorFrame_L",
            Box::new(Cube {
                width: door_frame_thick,
                height: door_total_h,
                depth: wall_thick + 0.1,
            }),
            trim_mat,
        );
        d_frame_l.transform.position = [
            -door_w / 2.0 - door_frame_thick / 2.0,
            door_total_h / 2.0,
            door_base_z,
        ]
        .into();
        ...
        // 门右框
        let mut d_frame_r = GameObject::new(
            "DoorFrame_R",
            Box::new(Cube {
                width: door_frame_thick,
                height: door_total_h,
        ...
        // 上下框
        let mut w_frame_top = GameObject::new(
            "WinFrame_Top",
            Box::new(Cube {
                width: wall_thick + 0.1,
                height: win_frame_thick,
                depth: win_total_w,
            }),
            trim_mat,
        );
        w_frame_top.transform.position =
            [win_base_x, win_y + win_h / 2.0 + win_frame_thick / 2.0, 0.0].into();
        self.add_object(w_frame_top);

        let mut w_frame_bot = GameObject::new(
            "WinFrame_Bot",
            Box::new(Cube {
                width: wall_thick + 0.1,
                height: win_frame_thick,
        ...
        w_frame_bot.transform.position =
            [win_base_x, win_y - win_h / 2.0 - win_frame_thick / 2.0, 0.0].into();
        self.add_object(w_frame_bot);

        // 左右框
        let mut w_frame_l = GameObject::new(
            "WinFrame_L",
            Box::new(Cube {
                width: wall_thick + 0.1,
                height: win_total_h,
                depth: win_frame_thick,
            }),
            trim_mat,
        );
        w_frame_l.transform.position =
            [win_base_x, win_y, -win_w / 2.0 - win_frame_thick / 2.0].into();
        self.add_object(w_frame_l);

        let mut w_frame_r = GameObject::new(
            "WinFrame_R",
            Box::new(Cube {
                width: wall_thick + 0.1,
                height: win_total_h,
        ...
        w_frame_r.transform.position =
            [win_base_x, win_y, win_w / 2.0 + win_frame_thick / 2.0].into();
        self.add_object(w_frame_r);

        // 玻璃
        let mut window = GameObject::new(
            "Window_Glass",
            Box::new(Cube {
                width: 0.05,
                height: win_h,
        ...
        for i in 0..5 {
            ...
            beam.transform.position =
                [0.0, room_h - 0.2, -room_d / 2.0 + 3.0 + i as f32 * 4.5].into();
        ...
        pillar2.transform.position =
            [-room_w / 2.0 + 1.2, room_h / 2.0, -room_d / 2.0 + 2.5].into();
        pillar2.set_body_type(BodyType::Static);
        self.add_object(pillar2);

        // 灯光
        let mut main_light = LightObject {
            name: "Main".to_string(),
            light: Light::POINT,
        };
        ...
        let mut target_light = LightObject {
            name: "TargetL".to_string(),
            light: Light::POINT,
        };
        ...
        if let Some(cam_idx) = self.get_selected_camera() {
            ...
            cam.transform
                .look_at([0.0, 2.0, 0.0].into(), [0.0, 1.0, 0.0].into());
            cam.move_state = crate::scene::camera::MoveState::RigidBody;

            // 加载武器模型
            if let Ok(weapon_mesh) =
                crate::geometry::shape::mesh::Mesh::load_obj("assets/models/weapons/rifle.obj")
            {
