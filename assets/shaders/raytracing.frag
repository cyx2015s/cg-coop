...
void getVec4FromTexture(sampler2D data, vec2 tex_coord, out vec4 v);
...
vec4 trace(inout Ray ray) {
    ...
    for (int depth = 0; depth < maxTraceDepth; depth++) {
        ...
        if (!intersect(ray, isect)) {
            // ������պ�
            radiance += throughput * texture(sky, vec3(ray.dir.x,-ray.dir.y,ray.dir.z)).rgb;
            break;
        }
        
        getMaterialData(materialBuffer, isect.primitive.materialIdx, isect.material);
        
        Material material = isect.material;

        if (material.type == LAMBERTIAN_MATERIAL) {
            if (!lambertianScatterFunction(ray, isect)) break;
            throughput *= material.albedo;
        } else if (material.type == METAL_MATERIAL) {
            if (!metalScatterFunction(ray, isect)) break;
            throughput *= material.albedo;
        } else if (material.type == DIELECTRIC_MATERIAL) {
...
void outputSample(vec4 color) {
    vec3 rst = texture(RTResult, screentex_coord).rgb;
...
bool intersect(inout Ray ray, inout Interaction isect) {
    ...
    while (stackPtr > 0) {
        ...
        if (node.nodeType == BVH_LEAF_NODE) {
            for (int i = 0; i < node.secondVal; i++) {
                Primitive primitive;
                getPrimitiveData(primitiveBuffer, node.firstVal + i, primitive);
            ...
            stack[stackPtr++] = node.secondVal;
            stack[stackPtr++] = node.firstVal;
        }
    }
...
bool intersectAABB(inout Ray ray, inout AABB box, inout vec3 invDir) {
    ...
    vec3 t0 = (box.pMin - ray.o) * invDir;
    vec3 t1 = (box.pMax - ray.o) * invDir;
    ...
    return tEnter <= tExit && tExit >= 0.0 && tEnter <= ray.tMax;
}
...
bool intersectPrimitive(inout Ray ray, inout Primitive primitive, inout Interaction isect) {
    ...
    if (primitive.shapeType == SPHERE_SHAPE) {
        ...
        return intersectSphere(ray, sphere, isect);
    } else if (primitive.shapeType == TRIANGLE_SHAPE) {
        TriangleIndex triIdx;
        getTriangleIndexData(triangleIndexBuffer, primitive.shapeIdx, triIdx);
        Triangle triangle;
        getTriangleData(vertexBuffer, triIdx, triangle);
        return intersectTriangle(ray, triangle, isect);
    }
    return false;
}
...
bool intersectSphere(inout Ray ray, inout Sphere sphere, inout Interaction isect) {
    ...
    if (t0 < 0.0) {
        t = t1;
        if (t1 < 0.0 || t1 > ray.tMax) {
            return false;
        }
    }
    
    vec3 hitPoint = getHitPoint(ray, t);
    vec3 normal = normalize(hitPoint - sphere.position);
    
    isect.hitPoint.position = hitPoint;
    isect.hitPoint.normal = normal;
    ray.tMax = t;
...
bool intersectTriangle(inout Ray ray, inout Triangle mesh, inout Interaction isect) {
    ...
    vec3 v0 = mesh.v[0].position;
    vec3 v1 = mesh.v[1].position;
    vec3 v2 = mesh.v[2].position;
    ...
    float w = 1.0 - u - v;
    vec3 normal = normalize(w * mesh.v[0].normal + 
                           u * mesh.v[1].normal + 
                           v * mesh.v[2].normal);
...
bool lambertianScatterFunction(inout Ray ray, inout Interaction isect) {
    ...
    vec3 normal = isect.hitPoint.normal;
    vec3 hitPoint = isect.hitPoint.position;
    
    vec2 u = rngGetRandom2D();
    vec3 scatterDir = toWorld(createLocalCoord(normal), uniformSampleHemiSphere(u));
    
    if (dot(scatterDir, normal) < 0.0) {
        scatterDir = reflect(scatterDir, normal);
    }
    
    ray.o = hitPoint + normal * Epsilon;
    ray.dir = scatterDir;
    ray.tMax = INFINITY;
...
bool metalScatterFunction(inout Ray ray, inout Interaction isect) {
    ...
    vec3 reflected = reflect(normalize(ray.dir), normal);
    
    if (isect.material.fuzz > 0.0) {
        reflected += isect.material.fuzz * uniformSampleSphere(rngGetRandom2D());
    }
...
void dielectricScatterFunction(inout Ray ray, inout Interaction isect) {
    vec3 normal = isect.hitPoint.normal;
    vec3 hitPoint = isect.hitPoint.position;
    float ior = isect.material.ior;

    // determine front face
    bool frontFace = dot(ray.dir, normal) < 0.0;
    vec3 n = frontFace ? normal : -normal;

    float etai_over_etat = frontFace ? (1.0 / ior) : ior;

    float cosTheta = min(dot(-ray.dir, n), 1.0);
    float sinTheta = sqrt(max(0.0, 1.0 - cosTheta * cosTheta));

    bool cannotRefract = etai_over_etat * sinTheta > 1.0;
    float reflectProb = fresnelSchlick(cosTheta, ior); 

    vec3 direction;
    if (cannotRefract || reflectProb > rngGetRandom1D()) {
        direction = reflect(ray.dir, n);
        ray.o = hitPoint + n * Epsilon;
    } else {
        ray.o = hitPoint - n * Epsilon;
        direction = refract(ray.dir, n, etai_over_etat);
    }
...
void rngInit() {
    rngState = texture(oldRngState, screentex_coord).r;
}
